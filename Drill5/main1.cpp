...
private:
  ...
  void change1(Line_style s) {lines.set_style(s);}
  void shide_menu() {style_menu.hide(); smenu_button.show();}
  void chide_menu() {color_menu.hide(); cmenu_button.show();}
  void red_pressed() {change(Color::red);}
  void blue_pressed() {change(Color::blue);}
  void black_pressed() {change(Color::black);}
  void dot_pressed() {change1(Line_style::dot);}
  void dash_pressed() {change1(Line_style::dash);}
  void solid_pressed() {change1(Line_style::solid);}
...
style_menu{Point{x_max()-70,100},80,20,Menu::vertical,"Style"},
smenu_button{Point{x_max()-80,110},80,20,"Style menu",cb_smenu}
{
  attach(color_menu);
  attach(style_menu);
  attach(next_button);
